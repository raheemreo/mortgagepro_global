...
class _CACmhcInsuranceState extends ConsumerState<CACmhcInsurance> {
  ...
  void _saveCalculation() async {
    final double price = double.tryParse(_priceController.text) ?? 650000;
    final double down = double.tryParse(_downController.text) ?? 65000;
          ...
          child: Column(
            children: [
              _buildInputField(
                ...
                onChanged: (val) {
                  final double pr = double.tryParse(val) ?? 0;
                  setState(() {
                    _downController.text = (pr * downPct / 100).round().toString();
                  });
                },
              ),
              const SizedBox(height: 8),
              Slider(
                value: price.clamp(100000, 1500000),
                min: 100000,
                max: 1500000,
                ...
                onChanged: (val) {
                  setState(() {
                    ...
                    _downController.text = (val * downPct / 100).round().toString();
                  });
                },
              ),
              const SizedBox(height: 12),
              _buildInputField(
                label: 'Down Payment',
                prefix: 'CA\$',
                suffix: '${downPct.toStringAsFixed(1)}%',
              ...
              Slider(
                ...
                onChanged: (val) {
                  setState(() {
                    _downController.text = (price * val / 100).round().toString();
                  });
                },
              ),
