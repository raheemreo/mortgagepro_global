...
class _NZPreApprovalGuideState extends ConsumerState<NZPreApprovalGuide> {
  ...
  void _saveCalculation() async {
    final double inc = double.tryParse(_incomeController.text) ?? 87000;
    final double partnerInc = double.tryParse(_partnerController.text) ?? 0;
    final double otherDebt = double.tryParse(_debtController.text) ?? 0;
    final double deposit = double.tryParse(_depositController.text) ?? 130000;

    final double totalInc = inc + partnerInc;
    final double stressRateDec = _stressRate / 100 / 12;
    final int n = _term * 12;
    ...
    final maxLoan = min(maxLoanByDTI, maxLoanByServiceability);
    ...
    if (confirmed == true && mounted) {
      ...
      final calc = SavedCalc.create(
        ...
        inputs: {
          'income': inc,
          'partnerIncome': partnerInc,
          'otherDebt': otherDebt,
          'deposit': deposit,
          'term': _term.toDouble(),
          'stressRate': _stressRate,
        },
        results: {
          'maxLoan': maxLoan,
          'dtiRatio': dti,
                      ...
                      children: [
                        Text('OTHER DEBTS (MONTHLY)',
                            style: AppTextStyles.dmSans(
                                ...
                                color: theme.getMutedColor(context))),
                        const SizedBox(height: 6),
                        _buildInputBox(_debtController),
                      ],
                    ),
                  ),
                  const SizedBox(width: 8),
