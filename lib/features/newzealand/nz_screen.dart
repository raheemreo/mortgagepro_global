              ...
              SliverPadding(
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    // Reserve Bank NZ Official Cash Rate
                    SectionLabel(
                      text: 'Reserve Bank NZ',
                      labelColor: _theme.mutedColor,
                      moreColor: _theme.primaryColor,
                      onMoreTap: () =>
                          context.push('/tool/newzealand/ocrhistory'),
                    ),
                    _RBNZBanner(),

                    // NZ Market Snapshot
                    SectionLabel(
                      text: 'NZ Market Snapshot',
                      labelColor: _theme.mutedColor,
                      moreColor: _theme.primaryColor,
                    ),
                    _NZMarketTicker(),

                    // Live Home Loan Rates Scrollable
                    SectionLabel(
                      text: 'Live Home Loan Rates',
                      moreText: 'Compare Lenders →',
                      labelColor: _theme.mutedColor,
                      moreColor: _theme.primaryColor,
                      onMoreTap: () =>
                          context.push('/tool/newzealand/comparelenders'),
                    ),
...
// ─── RBNZ OCR Banner ────────────────────────────────────────────────────────
class _RBNZBanner extends ConsumerWidget {
  @override
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'OCR RATE · RBNZ OFFICIAL CASH RATE',
...
// ─── NZ Market Snapshot ────────────────────────────────────────────────────
class _NZMarketTicker extends ConsumerWidget {
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final liveBadge = isLive ? '🟢' : '•';
    final items = [
      ('NZD/USD',   nzdUsd.toStringAsFixed(4), '$liveBadge ${isLive ? "Live" : "Est."}', isLive),
      ('NZD/AUD',   nzdAud.toStringAsFixed(4), '$liveBadge ${isLive ? "Live" : "Est."}', isLive),
      ('NZD/GBP',   nzdGbp.toStringAsFixed(4), '$liveBadge ${isLive ? "Live" : "Est."}', isLive),
      ('2-Yr Bond',  '${rc.nzBond2yr.toStringAsFixed(2)}%', 'Govt', false),
      ('10-Yr Bond', '${rc.nzBond10yr.toStringAsFixed(2)}%', 'Govt', true),
      ('CPI',        '${rc.nzCpi.toStringAsFixed(1)}%', '↓ Easing', true),
    ];
    return Container(
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('NZX & Economic Indicators',
                  style: AppTextStyles.infoTitle(theme.getTextColor(context))),
              Text(isLive ? '🟢 Live' : 'Remote Config',
                  style: AppTextStyles.dmSans(
                      size: 10, color: const Color(0xFF1A6B4A))),
            ],
          ),
          const SizedBox(height: 10),
          GridView.count(
...
class _MortgageRatesScroll extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final r = ref.watch(nzRatesProvider).valueOrNull;
    final isLive = r?.isLive == true;
    final chg = isLive ? '🟢 Live' : 'Est.';
    final rates = [
      ('🏠', '1-Yr Fixed', '${(r?.fixed1yr.value ?? 5.59).toStringAsFixed(2)}%', 'ANZ / ASB', chg, !isLive),
      ('📅', '2-Yr Fixed', '${(r?.fixed2yr.value ?? 5.29).toStringAsFixed(2)}%', 'Kiwibank',  chg, !isLive),
      ('📆', '3-Yr Fixed', '${(r?.fixed3yr.value ?? 5.19).toStringAsFixed(2)}%', 'Westpac',   chg, !isLive),
      ('📊', '5-Yr Fixed', '${(r?.fixed5yr.value ?? 5.09).toStringAsFixed(2)}%', 'BNZ / Avg', chg, !isLive),
      ('🔄', 'Floating',  '${(r?.floating.value  ?? 7.24).toStringAsFixed(2)}%', 'Variable',  chg, true),
      ('🔁', 'Revolving', '${((r?.floating.value ?? 7.24) + 0.10).toStringAsFixed(2)}%', 'Credit fac.', chg, true),
    ];
    return SizedBox(
      height: 130,
