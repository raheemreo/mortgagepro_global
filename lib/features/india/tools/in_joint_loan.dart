...
class _INJointLoanState extends ConsumerState<INJointLoan> {
  ...
  void _saveCalculation() async {
    final i1 = double.tryParse(_inc1Controller.text) ?? 120000;
    final i2 = double.tryParse(_inc2Controller.text) ?? 85000;
    final e1 = double.tryParse(_emi1Controller.text) ?? 0;
    final e2 = double.tryParse(_emi2Controller.text) ?? 0;
    ...
    if (confirmed == true && mounted) {
      ...
      final calc = SavedCalc.create(
        ...
        inputs: {
          'inc1': i1,
          'emi1': e1,
          'cibil1': double.tryParse(_cibil1Controller.text) ?? 750.0,
          'inc2': i2,
          'emi2': e2,
          'cibil2': double.tryParse(_cibil2Controller.text) ?? 750.0,
          'rate': rate,
  ...
  Widget build(BuildContext context) {
    ...
    final i1 = double.tryParse(_inc1Controller.text) ?? 0.0;
    final i2 = double.tryParse(_inc2Controller.text) ?? 0.0;
    final e1 = double.tryParse(_emi1Controller.text) ?? 0.0;
    final e2 = double.tryParse(_emi2Controller.text) ?? 0.0;
    ...
    final totalIncome = i1 + i2;
    final double pct1 = totalIncome > 0 ? (i1 / totalIncome) : 0.0;
    final double pct2 = totalIncome > 0 ? 1.0 - pct1 : 0.0;

    final f1 = i1 > 0 ? ((e1 + jointEmi / 2) / i1 * 100).round() : 0;
    final f2 = i2 > 0 ? ((e2 + jointEmi / 2) / i2 * 100).round() : 0;
    final fj = totalIncome > 0 ? ((e1 + e2 + jointEmi) / totalIncome * 100).round() : 0;
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  ...
                  _buildResultBox('Joint EMI', "₹${NumberFormat.currency(locale: 'en_IN', symbol: '', decimalDigits: 0).format(jointEmi)}", '$tenure-year tenure', false),
                ],
              ),
            ],
          ),
                    ...
                    child: Column(
                      children: [
                        _buildLegendRow(
                          ...
                          label: 'Primary',
                          value: "₹${NumberFormat.currency(locale: 'en_IN', symbol: '', decimalDigits: 0).format(i1)}",
                        ...
                        _buildLegendRow(
                          ...
                          label: 'Co-Applicant',
                          value: "₹${NumberFormat.currency(locale: 'en_IN', symbol: '', decimalDigits: 0).format(i2)}",
