...
class _AULmiCalcState extends ConsumerState<AULmiCalc> {
  ...
  String _ownerType = 'owner'; // 'owner', 'invest'
  ...
  // Premium scales based on LVR brackets (Australia standard Genworth guidelines)
  static const List<Map<String, dynamic>> _lmiTiers = [
    {
      'label': 'SAFE',
      'lvr': '≤ 80%',
      'rate': 0.0,
  ...
  void _calculate() {
    ...
    setState(() {
      ...
      _calcSnapshot['stateCode'] = _stateCode;
      _calcSnapshot['ownerType'] = _ownerType;
      _showResults = true;
    });
  ...
  Map<String, double> _calcLMI(double loan, double lvr, String owner) {
    if (lvr <= 80) {
      return {'lmi': 0.0, 'gst': 0.0, 'stamp': 0.0, 'total': 0.0, 'rate': 0.0};
    }

    double rate = 0;
    if (lvr <= 85) {
      rate = _lmiTiers[1]['rate'] as double;
    } else if (lvr <= 90) {
      rate = _lmiTiers[2]['rate'] as double;
    } else if (lvr <= 95) {
      rate = _lmiTiers[3]['rate'] as double;
    } else {
      rate = _lmiTiers[4]['rate'] as double;
    }

    if (owner == 'invest') {
      rate *= 1.15; // 15% loading for investment properties
    }
  ...
  void _saveCalculation() async {
    ...
    final String snapOwnerType = _calcSnapshot['ownerType'] ?? _ownerType;

    final loanAmt = snapPropVal - snapDeposit;
    if (loanAmt <= 0) return;

    final lvr = (loanAmt / snapPropVal) * 100;
    final lmiData = _calcLMI(loanAmt, lvr, snapOwnerType);
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Saving: LMI for \$${(snapPropVal / 1000).toStringAsFixed(0)}K property · LVR ${lvr.toStringAsFixed(1)}%',
    ...
    if (confirmed == true && mounted) {
      final label = labelCtrl.text.trim().isNotEmpty ? labelCtrl.text.trim() : 'LMI Calculation';
      final calc = SavedCalc.create(
        ...
        inputs: {
          ...
          'ownerType': snapOwnerType == 'owner' ? 0.0 : 1.0,
        },
        results: {
          'lvr': lvr,
          'depositPct': (snapDeposit / snapPropVal) * 100,
  ...
  Widget build(BuildContext context) {
    ...
    final String snapOwnerType = _showResults ? (_calcSnapshot['ownerType'] ?? _ownerType) : _ownerType;

    // Calculations
    final loanAmt = snapPropVal - snapDeposit;
    final lvr = snapPropVal > 0 ? (loanAmt / snapPropVal) * 100 : 0.0;
    final depPct = snapPropVal > 0 ? (snapDeposit / snapPropVal) * 100 : 0.0;
    ...
    double calcRepay(double loan) {
      ...
      return loan * (monthlyRate * pow(1 + monthlyRate, n)) / (pow(1 + monthlyRate, n) - 1);
    }
    ...
    final maxLMIValue = samples.map((p) => _calcLMI(p * (lvr / 100), lvr, snapOwnerType)['total']!).reduce(max);

    final isDirty = _showResults && (
      _propVal != (_calcSnapshot['propVal'] ?? 0.0) ||
      _deposit != (_calcSnapshot['deposit'] ?? 0.0) ||
      _termYears != (_calcSnapshot['termYears'] ?? 0) ||
      _stateCode != (_calcSnapshot['stateCode'] ?? '') ||
      _ownerType != (_calcSnapshot['ownerType'] ?? '')
    );
    ...
    return Column(
      ...
      children: [
            ...
            children: [
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text('STATE',
                              style: AppTextStyles.dmSans(size: 8.5, color: Colors.white54, weight: FontWeight.w600)),
                          ...
                          DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              items: ['NSW', 'VIC', 'QLD', 'WA', 'SA', 'TAS', 'ACT', 'NT'].map((st) {
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: _buildToggleButton('Owner Occupied', _ownerType == 'owner', () {
                        setState(() => _ownerType = 'owner');
                      }),
                    ),
                    Expanded(
                      child: _buildToggleButton('Investment', _ownerType == 'invest', () {
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Inputs have changed. Tap Calculate LMI to refresh results.',
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('LVR Gauge', style: AppTextStyles.playfair(size: 15, color: theme.getTextColor(context))),
                const SizedBox(height: 10),
                Container(
                  ...
                  child: Column(
                    children: [
                      Text('Loan-to-Value Ratio (LVR)', style: AppTextStyles.dmSans(size: 12, weight: FontWeight.w700, color: theme.getTextColor(context))),
                      const SizedBox(height: 14),
                      // Needle Gauge Track
                      LayoutBuilder(
                          ...
                          return Column(
                            ...
                            children: [
                              Stack(
                                ...
                                children: [
                                  Container(
                                    ...
                                    child: Row(
                                      children: [
                                        Expanded(flex: 80, child: Container(color: const Color(0xFF86EFAC))), // 60-80% safe
                                        Expanded(flex: 10, child: Container(color: const Color(0xFFFBBF24))), // 80-85% warn
                                        Expanded(flex: 5, child: Container(color: const Color(0xFFF97316))), // 85-90% med
                                        Expanded(flex: 5, child: Container(color: const Color(0xFFEF4444))), // 90-100% high
                                      ],
                                    ),
                                  ),
                                  Positioned(
                              ...
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text('60%', style: AppTextStyles.dmSans(size: 9, color: theme.getMutedColor(context))),
                                  Text('80%', style: AppTextStyles.dmSans(size: 9, color: theme.getMutedColor(context))),
                                  Text('85%', style: AppTextStyles.dmSans(size: 9, color: theme.getMutedColor(context))),
                                  Text('90%', style: AppTextStyles.dmSans(size: 9, color: theme.getMutedColor(context))),
                                  Text('95%+', style: AppTextStyles.dmSans(size: 9, color: theme.getMutedColor(context))),
                                ],
                              ),
                            ],
                          );
                          ...
                          _buildValBox(
                              ...
                              lvr <= 80
                                  ...
                                  : lvr <= 90
                                      ? Colors.orange
                                      : Colors.red),
                          _buildValBox('Deposit %', '${depPct.toStringAsFixed(1)}%', Colors.blue),
                          _buildValBox('Loan Amount', CurrencyFormatter.format(loanAmt, currencyCode: 'AUD'), Colors.black),
                        ],
                      ),
                    ],
                  ),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('LMI Result', style: AppTextStyles.playfair(size: 15, color: theme.getTextColor(context))),
                    GestureDetector(
                    ...
                    children: [
                      ...
                      Text(
                          ...
                          style: AppTextStyles.playfair(
                              size: 40,
                              color: lvr <= 80 ? Colors.green : (isDark ? const Color(0xFFFCA5A5) : Colors.red),
                              weight: FontWeight.w800)),
                      const SizedBox(height: 2),
                      Text(
                        lvr <= 80 ? '✅ No LMI required — LVR ≤ 80%' : 'LMI (${(rate * 100).toStringAsFixed(2)}%) + GST + Stamp Duty',
                      ...
                      if (lvr > 80) ...[
                        ...
                        Text(
                          ...
                          style: AppTextStyles.dmSans(size: 10, color: isDark ? Colors.white60 : Colors.grey),
                        ),
                      ],
                      const SizedBox(height: 16),
                      const Divider(),
                      ...
                      GridView.count(
                        ...
                        mainAxisSpacing: 10,
                        children: [
                          _buildSummaryBox('LMI Capitalised', CurrencyFormatter.format(total, currencyCode: 'AUD')),
                          _buildSummaryBox('Total Loan (incl. LMI)', CurrencyFormatter.format(loanAmt + total, currencyCode: 'AUD')),
                          _buildSummaryBox('Monthly Repayment', '${CurrencyFormatter.format(capRepay, currencyCode: 'AUD')}/mo'),
                          _buildSummaryBox('Extra Monthly (LMI)', '${CurrencyFormatter.format(extraMonthly, currencyCode: 'AUD')}/mo'),
                        ],
                      ),
                    ],
                  ),
                ...
                // Tiers Table Card
                const SizedBox(height: 20),
                Text('LMI Rate Tiers (Genworth 2026)', style: AppTextStyles.playfair(size: 15, color: theme.getTextColor(context))),
                const SizedBox(height: 10),
                Container(
                  ...
                  child: Column(
                    children: [
                      ..._lmiTiers.asMap().entries.map((entry) {
                        ...
                        final double displayRate = itemRate == 0 ? 0 : itemRate * (snapOwnerType == 'invest' ? 1.15 : 1.0);
                        ...
                        if (isDark) {
                          ...
                          badgeText = baseColor == const Color(0xFF7F1D1D) ? const Color(0xFFEF4444) : baseColor;
                        }

                        return Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 10),
                          decoration: BoxDecoration(
                            color: isActive ? (isDark ? const Color(0xFF7C2D12) : const Color(0xFFFFF8F0)) : Colors.transparent,
                            borderRadius: BorderRadius.circular(8),
                          ),
                          child: Row(
                            children: [
                              Container(
                                ...
                                child: Text(t['label'], style: AppTextStyles.dmSans(size: 9.5, weight: FontWeight.w800, color: badgeText)),
                              ),
                              const SizedBox(width: 14),
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('LVR ${t['lvr']}', style: AppTextStyles.dmSans(size: 11.5, weight: FontWeight.w700, color: theme.getTextColor(context))),
                                  Text(idx == 0 ? 'Standard requirement' : 'Premium of loan amount',
                              ...
                              Column(
                                ...
                                children: [
                                  Text(
                                    displayRate == 0 ? '—' : '${(displayRate * 100).toStringAsFixed(2)}%',
                                    style: AppTextStyles.dmSans(size: 12.5, weight: FontWeight.w800, color: rateColor),
                                  ),
                                  if (isActive)
                                    Text('◀ Your LVR',
                                        style: AppTextStyles.dmSans(
                      ...
                      Align(
                        ...
                        child: Text('LMI Cost Comparison', style: AppTextStyles.dmSans(size: 11, weight: FontWeight.w700, color: theme.getMutedColor(context))),
                      ),
                      const SizedBox(height: 10),
                      // Simple bar comparison
                      ...samples.map((p) {
                        final lmiVal = _calcLMI(p * (lvr / 100), lvr, snapOwnerType)['total']!;
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              SizedBox(
                                width: 85,
                                child: Text('\$${(p / 1000).toStringAsFixed(0)}K property', style: AppTextStyles.dmSans(size: 10, color: theme.getMutedColor(context))),
                              ),
                              Expanded(
                                child: Container(
                                  height: 10,
                                  ...
                                  child: FractionallySizedBox(
                                    ...
                                    child: Container(
                                      decoration: BoxDecoration(
                                        gradient: const LinearGradient(colors: [Color(0xFF002868), Color(0xFFEA580C)]),
                                        borderRadius: BorderRadius.circular(5),
                                      ),
                                    ),
                                  ),
                              ...
                              SizedBox(
                                width: 55,
                                child: Text(
                                  lmiVal > 0 ? CurrencyFormatter.compact(lmiVal, symbol: '\$') : '—',
        ...
        // About LMI Guide
        const SizedBox(height: 20),
        Text('About LMI', style: AppTextStyles.playfair(size: 15, color: theme.getTextColor(context))),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _buildTipRow('1', 'LMI protects the lender (not you) if you default. It\'s required by most lenders when LVR exceeds 80%.'),
              _buildTipRow('2', 'Rates are based on Genworth & QBE sliding scale tables — the larger and higher the LVR, the higher the premium.'),
              _buildTipRow('3', 'You can capitalise LMI (add to loan) but this means paying interest on the insurance premium over your whole loan term.'),
              _buildTipRow('4', 'First Home Guarantee (FHBG) lets eligible first-home buyers purchase with 5% deposit without paying LMI.'),
              _buildTipRow('5', 'Some professions (doctors, lawyers, accountants) get LMI waivers from certain lenders with up to 90% LVR.'),
            ],
          ),
        ),
      ],
  ...
  Widget _buildValBox(String label, String val, Color valCol) {
    ...
    return Column(
      children: [
        Text(label, style: AppTextStyles.dmSans(size: 9.5, color: isDark ? Colors.white70 : const Color(0xFF92400E))),
        const SizedBox(height: 2),
        Text(val, style: AppTextStyles.playfair(size: 16, weight: FontWeight.w800, color: displayColor)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          Text(value, style: AppTextStyles.playfair(size: 13, color: isDark ? Colors.white : Colors.black, weight: FontWeight.w800), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
