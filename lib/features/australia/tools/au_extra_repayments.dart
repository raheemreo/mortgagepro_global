...
class _AUExtraRepaymentsState extends ConsumerState<AUExtraRepayments> {
  ...
  String _freqExtra =
      'fortnightly'; // 'weekly', 'fortnightly', 'monthly', 'yearly'
  int _startFrom =
      0; // 0 (now), 12 (Year 1), 24 (Year 2), 36 (Year 3), 60 (Year 5)
  double _lumpSum = 0;
  ...
  void _saveCalculation() async {
    ...
    final stdPmt = _monthlyPmt(_loanBalance, _interestRate, _loanTerm);
    final extraMo = _extraPerMonth(_extraAmount, _freqExtra);

    // Standard
    double balStd = _loanBalance;
    double intStd = 0;
    for (int m = 1; m <= n; m++) {
      final i = balStd * r;
      intStd += i;
    ...
    // With extra
    double balNew = _loanBalance - _lumpSum;
    double intNew = 0;
    int newN = 0;
    ...
    for (int m = 1; m <= n + 120; m++) {
      ...
      final i = bal * r;
      intNew += i;
      final payment = stdPmt + (m > _startFrom ? extraMo : 0);
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
                'Saving: \$${CurrencyFormatter.compact(saved, symbol: 'AU\$')} saved · $yrsSaved yrs $moSaved mo cut',
  ...
  Widget build(BuildContext context) {
    ...
    final origTermLabel = '$_loanTerm years';
    final newTermLabel = (() {
      final y = newN ~/ 12;
      final m = newN % 12;
    ...
    for (int yr = 0; yr < yearsToShow; yr++) {
      ...
      final nwPrev =
          yr > 0 && yr - 1 < newYearly.length ? newYearly[yr - 1].intPaid : 0.0;
      ...
      final pctP = (1 - std.bal / _loanBalance).clamp(0.0, 1.0);
      final pctI = (intThisYrStd / (stdPmt * 12)).clamp(0.0, 1.0);
      final pctS =
          intThisYrStd > 0 ? (ySaving / intThisYrStd).clamp(0.0, 1.0) : 0.0;
      ...
      children: [
            ...
            children: [
                ...
                children: [
                  Expanded(
                      child: _buildInputBox(
                          'Loan Balance', 'AUD \$', _loanBalance,
                          onChanged: (v) => setState(() => _loanBalance = v))),
                  const SizedBox(width: 10),
                  Expanded(
                      child: _buildInputBox(
                          'Interest Rate', '% p.a.', _interestRate,
                          isPercent: true,
              ...
              Row(
                children: [
                  Expanded(
                      child: _buildInputBox(
                          'Remaining Term', 'yrs', _loanTerm.toDouble(),
                          isInteger: true,
                          onChanged: (v) =>
                              setState(() => _loanTerm = v.toInt()))),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                      child: _buildFreqBtn('Weekly', _freqExtra == 'weekly',
                          () => setState(() => _freqExtra = 'weekly'))),
                  const SizedBox(width: 4),
                  Expanded(
                      child: _buildFreqBtn(
                          'Fortnightly',
                          _freqExtra == 'fortnightly',
                          () => setState(() => _freqExtra = 'fortnightly'))),
                  const SizedBox(width: 4),
                  Expanded(
                      child: _buildFreqBtn('Monthly', _freqExtra == 'monthly',
                          () => setState(() => _freqExtra = 'monthly'))),
                  const SizedBox(width: 4),
                  Expanded(
                      child: _buildFreqBtn('Yearly', _freqExtra == 'yearly',
                          () => setState(() => _freqExtra = 'yearly'))),
                ],
              ),
              const SizedBox(height: 10),
              Row(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<int>(
                              ...
                              items: const [
                                ...
                                DropdownMenuItem(
                                    value: 12, child: Text('Year 1')),
                                DropdownMenuItem(
                                    value: 24, child: Text('Year 2')),
                                DropdownMenuItem(
                                    value: 36, child: Text('Year 3')),
                                DropdownMenuItem(
                                    value: 60, child: Text('Year 5')),
                              ],
                              onChanged: (v) {
        ...
        if (_showResults) ...[
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Your Savings Summary',
                  style:
                      AppTextStyles.playfair(size: 15, color: theme.textColor)),
              GestureDetector(
              ...
              children: [
                ...
                Text(CurrencyFormatter.format(saved, currencyCode: 'AUD'),
                    style: AppTextStyles.playfair(
                        ...
                        weight: FontWeight.w800)),
                Text('by making extra repayments',
                    style:
                ...
                GridView.count(
                  ...
                  mainAxisSpacing: 10,
                  children: [
                    _buildHeroBox('Time Saved', '$yrsSaved yrs $moSaved mo',
                        color: const Color(0xFFFFD700)),
                    _buildHeroBox('New Payoff Date', payoffFmt(payoffNew),
                        color: const Color(0xFFBBF7D0)),
                    _buildHeroBox('Old Repayment',
                        '${CurrencyFormatter.format(stdPmt, currencyCode: 'AUD')}/mo'),
                    _buildHeroBox('New Total/Period',
                        '${CurrencyFormatter.format(stdPmt + extraMo, currencyCode: 'AUD')}/mo',
                        color: const Color(0xFFFFD700)),
                  ],
                ),
              ],
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    _buildLegendItem(
                        const Color(0xFF16A34A), 'With Extra Repayments'),
                  ],
                ),
                const SizedBox(height: 16),
                ...
                Text('0 to $origTermLabel',
                    style: AppTextStyles.dmSans(
                        size: 10, color: theme.mutedColor)),
                const SizedBox(height: 10),

                // New Term bar
                _buildTermBar('New Term', newPct, const Color(0xFF16A34A)),
                Text('0 to $newTermLabel',
                    style: AppTextStyles.dmSans(
                        size: 10, color: theme.mutedColor)),
                ...
                Row(
                  children: [
                    Expanded(
                        child: _buildTimelineStat(
                            'Original Term', '$_loanTerm yrs')),
                    const SizedBox(width: 8),
                    Expanded(
                        child: _buildTimelineStat('New Term', newTermLabel,
                            isGreen: true)),
                    const SizedBox(width: 8),
                    Expanded(
                        child: _buildTimelineStat('Extra/Month',
                            '${CurrencyFormatter.format(extraMo, currencyCode: 'AUD')}/mo',
                            isGreen: true)),
                  ],
                ),
              ],
              ...
              children: [
                const Text('🎯 Key Milestones',
                    style: TextStyle(
                        ...
                        color: Colors.white)),
                const SizedBox(height: 12),
                _buildMilestoneRow('🏁', 'Loan paid off',
                    'Original: ${payoffFmt(payoffOrig)}', payoffFmt(payoffNew)),
                _buildMilestoneRow(
                    '💰',
                    'Total interest without extra',
                    'Standard repayments only',
                    CurrencyFormatter.format(intStd, currencyCode: 'AUD')),
                _buildMilestoneRow(
                    '✅',
                    'Total interest with extra',
                    'Reduced by extra repayments',
                    CurrencyFormatter.format(intNew, currencyCode: 'AUD')),
                _buildMilestoneRow(
                    '🎉',
                    'Total interest saved',
                    'Money back in your pocket',
                    CurrencyFormatter.format(saved, currencyCode: 'AUD')),
                _buildMilestoneRow('📅', 'Years cut from loan',
                    'Time freed up earlier', '$yrsSaved yrs $moSaved mo'),
              ],
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildLegendItem(
                        const Color(0xFF002868), 'Principal (Standard)'),
                    const SizedBox(width: 14),
                    _buildLegendItem(
                        const Color(0xFFFCA5A5), 'Interest (Standard)'),
                    const SizedBox(width: 14),
                    _buildLegendItem(
                        const Color(0xFFBBF7D0), 'Interest Saving'),
                  ],
                ),
                const SizedBox(height: 16),
                ...amortRows.map((ar) {
                    ...
                    child: Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                                '${CurrencyFormatter.format(ar.balance, currencyCode: 'AUD')} bal',
                                style: AppTextStyles.dmSans(
                                    size: 10,
                                    weight: FontWeight.bold,
                                    color: theme.textColor)),
                            if (ar.interestSaved > 0)
                              Text(
                                  '-${CurrencyFormatter.format(ar.interestSaved, currencyCode: 'AUD')} int',
  ...
  Widget _buildFreqBtn(String text, bool active, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 8),
        decoration: BoxDecoration(
          color: active ? const Color(0xFF7C2D12) : const Color(0xFFFFF8F0),
          border: Border.all(
              color:
                  active ? const Color(0xFF7C2D12) : const Color(0x3B7C2D12)),
  ...
  Widget _buildLegendItem(Color col, String text) {
    return Row(
      children: [
        ...
        Text(text,
            style: TextStyle(fontSize: 10, color: widget.theme.textColor)),
      ],
    );
  }
  ...
  Widget _buildTimelineStat(String label, String value,
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(label,
              style: const TextStyle(fontSize: 9, color: Color(0xFF92400E))),
          const SizedBox(height: 2),
          Text(value,
              style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.bold,
                  color: isGreen ? const Color(0xFF15803D) : Colors.black,
                  fontFamily: 'Georgia')),
        ],
      ),
    );
...
class _CumulativeInterestPainter extends CustomPainter {
  ...
  _CumulativeInterestPainter(
      {required this.stdYearly,
      required this.newYearly,
      required this.maxYrs,
      required this.maxV});
