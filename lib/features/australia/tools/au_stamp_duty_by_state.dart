...
class _AUStampDutyByStateState extends ConsumerState<AUStampDutyByState> {
  ...
  void _saveCalculation() async {
    ...
    final duty = _calculateDuty(snapPropVal, snapState, snapBuyerType);
    final effRate = snapPropVal > 0 ? (duty / snapPropVal * 100) : 0.0;
    final total = snapPropVal + duty + 3500;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Saving: \$${CurrencyFormatter.compact(duty, symbol: 'AU\$')} stamp duty in $snapState',
                          ...
                          : snapState == 'SA'
                              ...
                              : snapState == 'TAS'
                                  ...
                                  : snapState == 'ACT'
                                      ? 6.0
                                      : 7.0,
          'buyerTypeIndex': snapBuyerType == 'owner'
              ? 0.0
              : snapBuyerType == 'fhb'
                  ? 1.0
                  : 2.0,
  ...
  Widget _buildCalculatorTab(CountryTheme theme) {
    ...
    final duty = _calculateDuty(snapPropVal, snapState, snapBuyerType);
    final rawDutyWithoutConc = _calculateDuty(snapPropVal, snapState, 'owner');
    final concessionSaved = max(0.0, rawDutyWithoutConc - duty);
    final effRate = snapPropVal > 0 ? (duty / snapPropVal * 100) : 0.0;
    const estLegalOther = 3500.0;
    final totalCost = snapPropVal + duty + estLegalOther;

    final stateName = _stampRates[snapState]?['name'] ?? snapState;
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(child: _buildBuyerTab('Owner-Occ', _buyerType == 'owner', () => setState(() => _buyerType = 'owner'))),
                  const SizedBox(width: 4),
                  Expanded(child: _buildBuyerTab('First Home', _buyerType == 'fhb', () => setState(() => _buyerType = 'fhb'))),
                  const SizedBox(width: 4),
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: const [
                      DropdownMenuItem(value: 'established', child: Text('Established Home')),
                      DropdownMenuItem(value: 'new', child: Text('New Build / Plan')),
                      DropdownMenuItem(value: 'vacant', child: Text('Vacant Land')),
                    ],
                    onChanged: (val) {
                      if (val != null) {
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('$snapState — $stateName',
                          style: AppTextStyles.dmSans(size: 10, color: Colors.white60, weight: FontWeight.w800, letterSpacing: 0.8)),
                      const SizedBox(height: 4),
                      Text(CurrencyFormatter.format(duty, currencyCode: 'AUD'),
                          style: AppTextStyles.playfair(size: 34, color: Colors.white, weight: FontWeight.w900)),
                      Text('on a ${CurrencyFormatter.compact(snapPropVal, symbol: 'AU\$')} $snapPropType property',
                      ...
                      GridView.count(
                        ...
                        children: [
                          ...
                          _buildResultBox('Concession Saved', concessionSaved > 0 ? CurrencyFormatter.format(concessionSaved, currencyCode: 'AUD') : 'Nil'),
                          _buildResultBox('Total Purchase Cost', CurrencyFormatter.format(totalCost, currencyCode: 'AUD')),
                          _buildResultBox('Marginal Rate Bracket', '${_getMarginalRate(snapPropVal, snapState)}%'),
                        ],
                      ),
                      const SizedBox(height: 16),
                      ElevatedButton(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('Save Calculation', style: AppTextStyles.dmSans(size: 12, weight: FontWeight.w800)),
                          ],
                        ),
                      ),
                    ],
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('🎉 First Home Buyer Benefit',
                            style: AppTextStyles.playfair(size: 12, color: isDark ? const Color(0xFF93C5FD) : const Color(0xFF1E3A8A), weight: FontWeight.w800)),
                        const SizedBox(height: 3),
                        Text(_stampRates[snapState]['concessions'],
                            style: AppTextStyles.dmSans(size: 10, color: isDark ? const Color(0xFF60A5FA) : const Color(0xFF2563EB))),
                      ],
                    ),
                  ),
                ],
                  ...
                  child: Row(
                    children: [
                      SizedBox(
                        ...
                        child: CustomPaint(
                          painter: _StampDutyDonutPainter(
                            propertyVal: snapPropVal,
                            stampDuty: duty,
                            legalOther: estLegalOther,
                            primaryColor: isDark ? const Color(0xFF60A5FA) : theme.primaryColor,
                            accentColor: isDark ? const Color(0xFF34D399) : theme.accentColor,
                      ...
                      Expanded(
                        child: Column(
                          children: [
                            _buildLegendRow('Property Value', snapPropVal, theme.accentColor),
                            _buildLegendRow('Stamp Duty', duty, theme.primaryColor),
                            _buildLegendRow('Est. Legal & Other', estLegalOther, Colors.amber),
                          ],
                        ),
                      ),
                    ],
                ...
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Marginal Tax Rate Breakdown', style: AppTextStyles.dmSans(size: 12, weight: FontWeight.w800, color: theme.getTextColor(context))),
                      const SizedBox(height: 10),
                      _buildBreakdownTable(snapPropVal, snapState, theme),
                    ],
                  ),
                ),
              ],
        ...
        children: [
          ...
          Expanded(
            child: Text(label,
                style: AppTextStyles.dmSans(
                    size: 10.5,
                    color: widget.theme.getTextColor(context).withValues(alpha: 0.75),
                    weight: FontWeight.w600)),
          ),
          Text(CurrencyFormatter.format(val, currencyCode: 'AUD'),
              style: AppTextStyles.dmSans(size: 11, color: widget.theme.getTextColor(context), weight: FontWeight.w800)),
        ],
      ),
    );
  }
  ...
  Widget _buildBreakdownTable(double val, String stateKey, CountryTheme theme) {
    if (stateKey == 'NT') {
      return Text(
        ...
        style: AppTextStyles.dmSans(size: 11, color: theme.getTextColor(context), height: 1.4),
      );
    }
    ...
    for (var b in brackets) {
      ...
      final endText = maxV == double.infinity ? '+' : '– \$${CurrencyFormatter.compact(maxV, symbol: '')}';
          ...
          child: Row(
            ...
            children: [
              ...
              Text('$rate%',
                  style: AppTextStyles.dmSans(
                      size: 11,
                      color: Theme.of(context).brightness == Brightness.dark ? const Color(0xFFFFD700) : theme.primaryColor,
                      weight: FontWeight.w800)),
              Text(CurrencyFormatter.format(base + (taxable * rate / 100), currencyCode: 'AUD'),
  ...
  Widget _buildCompareTab(CountryTheme theme) {
    ...
    final List<Map<String, dynamic>> dataList = statesList.map((st) {
      final double dVal = _calculateDuty(_comparePropVal, st, _compareBuyerType);
      final double rate = _comparePropVal > 0 ? (dVal / _comparePropVal * 100) : 0.0;
      return {
        'state': st,
        'duty': dVal,
        'rate': rate,
    ...
    dataList.sort((a, b) => (a['duty'] as double).compareTo(b['duty'] as double));
      ...
      children: [
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(child: _buildBuyerTab('Owner-Occ', _compareBuyerType == 'owner', () => setState(() => _compareBuyerType = 'owner'))),
                  const SizedBox(width: 4),
                  Expanded(child: _buildBuyerTab('First Home', _compareBuyerType == 'fhb', () => setState(() => _compareBuyerType = 'fhb'))),
                  const SizedBox(width: 4),
                  Expanded(child: _buildBuyerTab('Investor', _compareBuyerType == 'investor', () => setState(() => _compareBuyerType = 'investor'))),
                ],
              ),
            ],
          ),
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Stamp Duty Payable (\$ AUD) 🗺️', style: AppTextStyles.dmSans(size: 12, weight: FontWeight.w800, color: theme.getTextColor(context))),
              const SizedBox(height: 12),
              ...dataList.map((item) {
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Container(
                          height: 16,
                          decoration: BoxDecoration(color: theme.getBgColor(context), borderRadius: BorderRadius.circular(4)),
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Effective Duty Rate (%)', style: AppTextStyles.dmSans(size: 12, weight: FontWeight.w800, color: theme.getTextColor(context))),
              const SizedBox(height: 12),
              ...dataList.map((item) {
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: FractionallySizedBox(
                            ...
                            child: Container(
                              decoration: BoxDecoration(
                                gradient: const LinearGradient(colors: [Colors.amber, Color(0xFFD97706)]),
                                borderRadius: BorderRadius.circular(4),
                              ),
                            ),
                          ),
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('$st — $name', style: AppTextStyles.playfair(size: 13, weight: FontWeight.bold, color: theme.getTextColor(context))),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                    decoration: BoxDecoration(
                        color: (isDark ? const Color(0xFFFFD700) : theme.primaryColor).withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(12)),
                    child: Text('2024–25',
                        style: AppTextStyles.dmSans(
                            size: 9,
              ...
              if (st == 'NT')
                Padding(
                  ...
                  child: Text(
                    ...
                    style: AppTextStyles.dmSans(size: 10.5, color: theme.getTextColor(context).withValues(alpha: 0.8), height: 1.4),
                  ),
                )
              else
                ...brackets.map((b) {
                  ...
                  final double base = b[2];
                  final double rate = b[3];
                  final endText = maxV == double.infinity ? 'and over' : '– \$${CurrencyFormatter.compact(maxV, symbol: '')}';
                  return Padding(
                    padding: const EdgeInsets.symmetric(vertical: 4),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('\$${CurrencyFormatter.compact(minV, symbol: '')} $endText',
                            style: AppTextStyles.dmSans(size: 10.5, color: theme.getTextColor(context).withValues(alpha: 0.75))),
                        Text(base > 0 ? '\$${CurrencyFormatter.compact(base, symbol: '')} + $rate%' : '$rate%',
                            style: AppTextStyles.dmSans(
                                size: 11,
                                weight: FontWeight.w800,
                                color: isDark ? const Color(0xFFFFD700) : theme.primaryColor)),
                ...
                children: [
                  ...
                  Expanded(
                    child: Text('FHB Concessions: $conc',
                        style: AppTextStyles.dmSans(size: 10, color: theme.getTextColor(context).withValues(alpha: 0.65))),
                  ),
                ],
              ),
            ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: AppTextStyles.dmSans(size: 12.5, color: Colors.white, weight: FontWeight.w800), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
...
class _StampDutyDonutPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final textPainter = TextPainter(
      text: TextSpan(text: '${pct.toStringAsFixed(1)}%', style: AppTextStyles.dmSans(size: 11, weight: FontWeight.w900, color: textColor)),
