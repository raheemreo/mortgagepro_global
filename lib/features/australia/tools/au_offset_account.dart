...
class _AUOffsetAccountState extends ConsumerState<AUOffsetAccount> {
  ...
  void _saveCalculation() async {
    ...
    final effectiveLoan = max(0.0, _loanBal - _offsetBal);

    final monthlyNo = _calcMonthly(_loanBal, r, n);
    final monthlyYes = _calcMonthly(effectiveLoan, r, n) + _monthlyContrib;
    final intNo = _calcTotalInterest(_loanBal, r, n);
    final intYes = _calcTotalInterest(effectiveLoan, r, n);
    final totalSaved = max(0.0, intNo - intYes);
    final effRate =
        effectiveLoan > 0 ? (_rate * effectiveLoan / _loanBal) : 0.0;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
                'Saving: \$${CurrencyFormatter.compact(totalSaved, symbol: 'AU\$')} saved off mortgage',
    ...
    if (confirmed == true && mounted) {
      ...
      final calc = SavedCalc.create(
        ...
        inputs: {
          ...
          'monthlyContrib': _monthlyContrib,
        },
        results: {
          'totalSaved': totalSaved,
          'monthlySave': max(0.0, monthlyNo - monthlyYes + _monthlyContrib),
  ...
  Widget build(BuildContext context) {
    ...
    final intNo = _calcTotalInterest(_loanBal, r, n);
    final intYes = _calcTotalInterest(effectiveLoan, r, n);
    final totalNo = _loanBal + intNo;
    final totalYes = effectiveLoan + intYes;
    final monthlySave = max(0.0, monthlyNo - monthlyYes + _monthlyContrib);
    final totalSaved = max(0.0, intNo - intYes);
    final effRate =
        effectiveLoan > 0 ? (_rate * effectiveLoan / _loanBal) : 0.0;
    ...
    for (int m = 0; m <= n; m += step) {
      ...
      for (int i = 0; i < step; i++) {
        ...
        final iYes = balYes * r;
        final pYes =
            monthlyNo - iYes; // using base repayment to see payoff speedup
        ...
        if (_showResults) ...[
              ...
              children: [
                ...
                Text(CurrencyFormatter.format(totalSaved, currencyCode: 'AUD'),
                    style: AppTextStyles.playfair(
                        ...
                        weight: FontWeight.w800)),
                Text('over your remaining loan term',
                    style:
                  ...
                  children: [
                    ...
                    Expanded(
                        child: _buildHeroStatBox(
                            'Years Saved',
                            yearsSaved > 0
                                ? '${yearsSaved.toStringAsFixed(1)} yrs'
                                : '< 1 yr')),
                    const SizedBox(width: 8),
                    Expanded(
                        child: _buildHeroStatBox('Effective Rate',
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                _buildCompRow(
                    'Monthly Repay',
                    '${CurrencyFormatter.format(monthlyNo, currencyCode: 'AUD')}/mo',
                    '${CurrencyFormatter.format(_calcMonthly(effectiveLoan, r, n), currencyCode: 'AUD')}/mo',
                    isGreenHighlight: true),
                _buildCompRow(
                    'Total Interest',
                    CurrencyFormatter.format(intNo, currencyCode: 'AUD'),
                    CurrencyFormatter.format(intYes, currencyCode: 'AUD'),
                    isGreenHighlight: true),
                _buildCompRow(
                    'Total Repaid',
                    CurrencyFormatter.format(totalNo, currencyCode: 'AUD'),
                    CurrencyFormatter.format(totalYes, currencyCode: 'AUD'),
                    isGreenHighlight: true),
                _buildCompRow(
                    'Effective Balance',
                    CurrencyFormatter.format(_loanBal, currencyCode: 'AUD'),
                    CurrencyFormatter.format(effectiveLoan,
                        currencyCode: 'AUD'),
                    isGreenHighlight: true),
              ],
            ),
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Loan Balance Trajectory',
                    style: AppTextStyles.dmSans(
                        size: 13,
                        weight: FontWeight.w800,
                        color: theme.getTextColor(context))),
                Text('With vs without offset account over time',
                    style: AppTextStyles.dmSans(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        yearsSaved > 0
                            ? 'Save ${yearsSaved.toStringAsFixed(1)} years off your loan!'
                            : 'Great start — keep growing your offset!',
                        style: AppTextStyles.playfair(
                            size: 15,
                            color: isDark
                                ? const Color(0xFF6EE7B7)
                      ...
                      Text(
                        'By maintaining \$${CurrencyFormatter.compact(_offsetBal)} in offset + \$${CurrencyFormatter.compact(_monthlyContrib)}/mo contributions',
                        style: AppTextStyles.dmSans(
                            size: 11,
                            color: isDark
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _buildTipRow('1',
                  'Every dollar in your offset reduces the loan balance used to calculate interest — effectively earning your mortgage rate tax-free.'),
              _buildTipRow('2',
                  'In 2026, best HISA rates sit around 5.00%. Your mortgage at 6.09% means offset beats savings by ~1.09% (pre-tax equivalent: ~1.56% for a 30% taxpayer).'),
              _buildTipRow('3',
                  'Park your salary in the offset before paying bills — even a few days\' difference saves measurable interest over time.'),
              _buildTipRow('4',
                  '100% offset accounts (most common in Australia) give full dollar-for-dollar reduction. Partial offset accounts only reduce a portion — read the fine print.'),
            ],
          ),
        ),
      ],
  ...
  Widget _buildCompRow(String metric, String noVal, String yesVal,
      {required bool isGreenHighlight}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Container(
    ...
    return Row(
      children: [
        Container(
          ...
          height: 3,
          decoration:
              BoxDecoration(color: col, borderRadius: BorderRadius.circular(2)),
        ),
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(text,
                style: AppTextStyles.dmSans(
                    size: 11, color: const Color(0xFF065F46), height: 1.4)),
          ),
        ],
      ),
    );
